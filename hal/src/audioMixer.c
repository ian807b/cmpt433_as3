...
void AudioMixer_init(void) {
  ...
  //     sound bite.
  // for loop to iterate through soundBites
  // soundBits[i]->pSound = NULL
...
void AudioMixer_queueSound(wavedata_t *pSound) {
  ...
  if (!freeSound) {
    printf("Error: Could not add sound into queue, try again\n");
  }
...
static void fillPlaybackBuffer(short *buff, int size) {
  ...
  memset(buff, 0, size);
  pthread_mutex_lock(&audioMutex);
  // main for loop to iterate through buffer
  for (int buff_index = 0; buff_index < size; buff_index++) {
    short total_pcm = 0;
    for (int i = 0; i < MAX_SOUND_BITES; i++) {
      wavedata_t *local_sound = soundBites[i].pSound;
      if (local_sound != NULL) {
        int local_location = soundBites[i].location;
        if (local_location < local_sound->numSamples) {
          short pcm = local_sound->pData[local_location];
          if (pcm >= SHRT_MAX) {
            pcm = SHRT_MAX;
          } else if (pcm <= SHRT_MIN) {
            pcm = SHRT_MIN;
          }
          // sound bite found
          total_pcm += pcm;
          ...
          local_location = local_sound->numSamples;
        }
        ...
        if (local_location >= local_sound->numSamples) {
          ...
          // update the location
          soundBites[i].location = local_location;
        }
      }
    }
    if (total_pcm >= SHRT_MAX) {
      total_pcm = SHRT_MAX;
    } else if (total_pcm <= SHRT_MIN) {
      total_pcm = SHRT_MIN;
    }
    // fill in buff for position: buff_index
    buff[buff_index] = total_pcm;
  }
  pthread_mutex_unlock(&audioMutex);
}
...
void *playbackThread(void *arg) {
  ...
  while (!stopping) {
    ...
    Period_markEvent(PERIOD_EVENT_SAMPLE_PBUFFER);
    // mark event
